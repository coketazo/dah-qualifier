...
현재 mock은 `MONITORING → GNSS_QUARANTINED → EXTERNAL_NAV_RTL`과 **`GNSS_QUARANTINED → SAFE_HOLD`** 분기를 모두 구현한다. blue 판단층은 독립 `ODOMETRY` 공분산(품질)을 보고, 양호하면 ExternalNav RTL, 불충분하면 나쁜 항법원으로 자동복귀하지 않고 `SAFE_HOLD`(안전 LOITER + 운용자 인계, `nav_source=INERTIAL_HOLD`)를 선택한다. LAND/다신호 종합 등 추가 분기는 본선 확장이다.
...
5. ~~LLM-only red/blue agent를 동일 공개 계약에 연결~~ → **구현 완료**(`src/red_agent`, `src/blue_agent`, 교전 하네스 `demo.run_agent_engagement`). 남은 것은 상용 API 백엔드를 온프레미스 Hermes로 스왑하는 에어갭 배치.
